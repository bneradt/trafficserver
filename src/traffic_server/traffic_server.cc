...
/**
 * @brief Create an appropriate PluginInfo object.
 *
 ...
 * @param[in] plugin_path That path to the plugin's shared object file.
 * @param[in] runtime_directory The directory the PluginInfo will use to
 *   verify the plugin.
 *
 * @return a PluginInfo object.
 */
static std::function<bool(std::string &)>
get_plugin_load_function(plugin_type_t plugin_type, const fs::path &plugin_path)
{
  switch (plugin_type) {
  case plugin_type_t::GLOBAL:
    return [plugin_path](std::string &error) {
      void *handle, *initptr;
      bool loaded = plugin_dso_load(plugin_path.c_str(), handle, initptr, error);
      if (loaded) {
        dlclose(handle);
      }
      return loaded;
  ...
  case plugin_type_t::REMAP:
    ...
    if (!fs::create_directories(temporary_directory, ec)) {
      return [temporary_directory, ec](std::string &error) {
        error = std::string("Could not create temporary directory %s: %s\n") + temporary_directory.string() + ec.message();
        return false;
      };
    }
    const auto runtime_path = temporary_directory / ts::file::filename(plugin_path);
    const fs::path unused_config;
    // Conceptually, a std::unique_ptr would make sense here, but std::function
    // has a copy-able requirement and std::unique_ptr is not copyable by
    // design. To satisfy std::function, we use std::shared_ptr.
    auto plugin_info = std::make_shared<RemapPluginInfo>(unused_config, plugin_path, runtime_path);
    return [temporary_directory, plugin_info = plugin_info](std::string &error) {
  ...
  return [](std::string &error) {
    error = "Could not determine plugin type";
    return false;
  };
}

/**
 * @brief A helper for the verify plugin command functions.
 *
 * @param[in] args The arguments passed to the -C command option. This includes
 * verify_global_plugin.
 *
...
verify_plugin_helper(char *args, plugin_type_t plugin_type)
{
  ...
  auto loader = get_plugin_load_function(plugin_type, plugin_path);
...
/**
 * @brief Verify whether a given SO file looks like a valid global plugin.
 *
 * @param[in] args The arguments passed to the -C command option. This includes
 * verify_global_plugin.
 *
...
/**
 * @brief Verify whether a given SO file looks like a valid remap plugin.
 *
 * @param[in] args The arguments passed to the -C command option. This includes
 * verify_global_plugin.
 *
