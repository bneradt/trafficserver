...
#else
#define sdk_assert(EX) ((void)((EX) ? (void)0 : tsapi::c::_TSReleaseAssert(#EX, __FILE__, __LINE__)))
#endif
...
extern ClassAllocator<tsapi::c::INKContInternal> INKContAllocator;
extern ClassAllocator<tsapi::c::INKVConnInternal> INKVConnAllocator;
...
sdk_alloc_field_handle(tsapi::c::TSMBuffer /* bufp ATS_UNUSED */, MIMEHdrImpl *mh)
{
  ...
  sdk_assert(tsapi::c::sdk_sanity_check_null_ptr((void *)handle) == tsapi::c::TS_SUCCESS);
...
static void
sdk_free_field_handle(tsapi::c::TSMBuffer bufp, MIMEFieldSDKHandle *field_handle)
{
  if (sdk_sanity_check_mbuffer(bufp) == tsapi::c::TS_SUCCESS) {
...
namespace c
{
  ...
  */
  ...
  TSUrlFtpTypeSet(TSMBuffer bufp, TSMLoc obj, int type)
  {
    ...
    if ((type == 0 || type == 'A' || type == 'E' || type == 'I' || type == 'a' || type == 'i' || type == 'e') &&
        isWriteable(bufp)) {
      URL u;
  ...
  TSFetchSM
  TSFetchCreate(TSCont contp, const char *method, const char *url, const char *version, struct sockaddr const *client_addr,
                int flags)
  {
    sdk_assert(sdk_sanity_check_continuation(contp) == TS_SUCCESS);
    sdk_assert(ats_is_ip(client_addr));
  ...
  */
  ...
  static const void *
  _conf_to_memberp(TSOverridableConfigKey conf, const OverridableHttpConfigParams *overridableHttpConfig,
