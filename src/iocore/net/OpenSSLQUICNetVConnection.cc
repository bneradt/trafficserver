...
public:
  ...
  QUICConnectionErrorUPtr create_uni_stream(QUICStreamId &new_stream_id) override;
  QUICConnectionErrorUPtr create_bidi_stream(QUICStreamId &new_stream_id) override;

private:
  QUICConnectionErrorUPtr _create_stream(QUICStreamId &new_stream_id, uint64_t flags);
...
QUICNetVConnection::acceptEvent(int event, Event *e)
{
  ...
  this->_stream_manager  = new OpenSSLQUICStreamManager(this->_context.get(), this->_application_map, this);
...
bool
QUICNetVConnection::_create_openssl_stream(uint64_t flags, QUICStreamId &stream_id)
{
  SSL *stream_ssl = SSL_new_stream(this->_ssl, flags);
  if (stream_ssl == nullptr) {
    QUICConDebug("Failed to create OpenSSL QUIC stream: %s", ERR_error_string(ERR_peek_error(), nullptr));
    return false;
  }
  ...
  stream_id          = SSL_get_stream_id(stream_ssl);
  auto [_, inserted] = this->_openssl_streams.emplace(stream_id, stream_ssl);
  if (!inserted) {
    SSL_free(stream_ssl);
    QUICConDebug("OpenSSL QUIC stream %" PRIu64 " already exists", stream_id);
    return false;
  }
...
OpenSSLQUICStreamManager::create_uni_stream(QUICStreamId &new_stream_id)
{
  return this->_create_stream(new_stream_id, SSL_STREAM_FLAG_UNI);
}

QUICConnectionErrorUPtr
OpenSSLQUICStreamManager::create_bidi_stream(QUICStreamId &new_stream_id)
{
  return this->_create_stream(new_stream_id, 0);
}

QUICConnectionErrorUPtr
OpenSSLQUICStreamManager::_create_stream(QUICStreamId &new_stream_id, uint64_t flags)
{
  if (this->_vc == nullptr || !this->_vc->_create_openssl_stream(flags, new_stream_id)) {
    return std::make_unique<QUICConnectionError>(QUICTransErrorCode::STREAM_LIMIT_ERROR, "Could not create OpenSSL QUIC stream");
  }
