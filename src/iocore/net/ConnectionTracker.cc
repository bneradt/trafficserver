...
ConnectionTracker::config_init(GlobalConfig *global, TxnConfig *txn)
{
  ...
  Enable_Config_Var(CONFIG_CLIENT_VAR_ALERT_DELAY, &Config_Update_Conntrack_Client_Alert_Delay, global);
  ...
  Enable_Config_Var(CONFIG_SERVER_VAR_ALERT_DELAY, &Config_Update_Conntrack_Server_Alert_Delay, global);
}
...
ConnectionTracker::Group::Group(DirectionType direction, Key const &key, std::string_view fqdn, int min_keep_alive)
  : _direction{direction},
    ...
    min_keep_alive_conns(min_keep_alive),
...
{
  Metrics::increment(net_rsb.connection_tracker_table_size);
...
{
  Metrics::decrement(net_rsb.connection_tracker_table_size);
}
