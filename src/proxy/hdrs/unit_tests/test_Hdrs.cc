...
// was changed via MIME layer functions, causing host_get() to return a string with
// the wrong length (reading garbage bytes past the actual hostname).
TEST_CASE("HdrTestHostCacheInvalidation", "[proxy][hdrtest]")
{
  ...
  SECTION("Host header modification invalidates cache")
  {
    ...
    REQUIRE(host1 != nullptr);
    REQUIRE(host_len1 == 30);
    REQUIRE(strncmp(host1, "very.long.hostname.example.com", 30) == 0);
    ...
    // BUG: Without the fix, this returns the old cached length (30)
    ...
    CHECK(host2 != nullptr);
    CHECK(host_len2 == 9);
    CHECK(strncmp(host2, "short.com", 9) == 0);
  ...
  SECTION("Host header modification to longer value")
  {
    ...
    const char *host1     = req_hdr.host_get(&host_len1);
    REQUIRE(host1 != nullptr);
    REQUIRE(strncmp(host1, "short.com", 9) == 0);
    ...
    CHECK(host2 != nullptr);
    CHECK(host_len2 == 30);
    CHECK(strncmp(host2, "very.long.hostname.example.com", 30) == 0);
  ...
  SECTION("Multiple Host header modifications")
  {
    ...
    REQUIRE(strncmp(host, "first.example.com", 17) == 0);
    ...
    CHECK(host != nullptr);
    CHECK(host_len == 10);
    CHECK(strncmp(host, "second.com", 10) == 0);
    ...
    CHECK(host != nullptr);
    CHECK(host_len == 27);
    CHECK(strncmp(host, "third.very.long.example.org", 27) == 0);
  ...
  SECTION("Host header with port - modify hostname only")
  {
    ...
    REQUIRE(strncmp(host1, "original.example.com", 20) == 0);
    REQUIRE(port1 == 8080);
  ...
  SECTION("Host header without port - add port")
  {
    ...
    CHECK(host2 != nullptr);
    CHECK(host_len2 == 11);
    CHECK(strncmp(host2, "newhost.com", 11) == 0);
    CHECK(port2 == 3128);
  ...
  SECTION("Host header with port - remove port")
  {
    ...
    CHECK(host2 != nullptr);
    CHECK(host_len2 == 18);
    CHECK(strncmp(host2, "noport.example.org", 18) == 0);
  ...
  SECTION("Multiple Host modifications with ports")
  {
    ...
    CHECK(host != nullptr);
    CHECK(host_len == 27);
    CHECK(strncmp(host, "very.long.third.example.org", 27) == 0);
    CHECK(port == 0);
