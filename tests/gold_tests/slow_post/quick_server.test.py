...
class QuickHttp2Test:
    """Verify that ATS doesn't delay responses behind slow HTTP/2 posts."""
    ...
    def _configure_dns(self, tr: 'TestRun') -> None:
        ...
        """
        self._dns = tr.MakeDNServer(
            f'h2-dns-{QuickHttp2Test._dns_counter}',
            default='127.0.0.1')
        QuickHttp2Test._dns_counter += 1
    ...
    def _configure_server(self, tr: 'TestRun'):
        ...
        :param tr: The test run to associate with the server process with.
        """
        cert = os.path.join(tr.Variables.AtsTestToolsDir, "ssl", "server.pem")
        key = os.path.join(tr.Variables.AtsTestToolsDir, "ssl", "server.key")
        tr.Setup.CopyAs(cert, tr.RunDirectory)
        tr.Setup.CopyAs(key, tr.RunDirectory)

        server = tr.Processes.Process(f'openssl-server-{QuickHttp2Test._server_counter}')
        QuickHttp2Test._server_counter += 1
        port = get_port(server, "https_port")
        server.Command = \
            f'echo HTTP | openssl s_server -accept {port} -cert server.pem -key server.key'
        self._server = server
    ...
    def _configure_traffic_server(self, tr: 'TestRun'):
        ...
        :param tr: The test run to associate with the ATS process with.
        """
        proc_name = f'ts-{QuickHttp2Test._ts_counter}'
        self._ts = tr.MakeATSProcess(proc_name, enable_tls=True)
        self._ts.addDefaultSSLFiles()
        QuickHttp2Test._ts_counter += 1
        self._ts.Disk.remap_config.AddLine(
            f'map / https://quick.server.com:{self._server.Variables.https_port}'
        )
    ...
    def run(self):
        ...
        content = 'a' * 1024 * 1024
        content_file = os.path.join(Test.RunDirectory, 'post_content')
        with open(content_file, 'w') as f:
            f.write(content)
        client_command = (
            f'curl -vk --limit-rate 1 --data-binary @{content_file} '
            f'https://127.0.0.1:{self._ts.Variables.ssl_port}/'
        )
        tr.Processes.Default.Command = client_command
