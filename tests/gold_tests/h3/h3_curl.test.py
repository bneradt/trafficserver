...
def wait_for_log_expression(log_path: str, expression: str) -> str:
    """Build a shell command that waits for an expression in a log file."""
    attempts = " ".join(str(i) for i in range(60))
    script = (
        "unset GREP_OPTIONS; "
        f"for _ in {attempts}; do grep -Eq -- {shlex.quote(expression)} {shlex.quote(log_path)} && exit 0; "
        "sleep 1; done; exit 1")
...
'''.split("\n"))
    ...
    def _configure_client(self):
        ...
        tr.Processes.Default.Command = wait_for_log_expression(
