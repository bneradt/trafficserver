...
tr.Processes.Default.Command = (f'cat {ts.Variables.LOGDIR}/crash-*.log 2>&1 | head -100')
tr.Processes.Default.ReturnCode = 0
...
# The crash log should enumerate threads (present even if ptrace fails).
tr.Processes.Default.Streams.stdout = Testers.ContainsExpression("Thread", "Expected crash log to contain thread information")
