...
ts.Disk.records_config.update(
    {
        ...
        'proxy.config.ssl.TLSv1_3.enabled': 1,
        'proxy.config.ssl.server.version.min': 1,
        'proxy.config.ssl.server.version.max': 3,
        'proxy.config.ssl.server.groups_list': 'X25519:P-256:X448:P-521:P-384:X25519MLKEM768',
...
logging:
  formats:
    - name: testformat
      format: '%<cqssu> %<pssc> %<pquc> %<pscert> %<cscert>'
  logs:
    - mode: ascii
      format: testformat
      filename: squid
...
tr.StillRunningAfter = ts
tr.StillRunningAfter = server
#"-vk --cert ./server.pem --key ./server.key --resolve 'foo.com:{0}:127.0.0.1' https://foo.com:{0}/case3"
tr.MakeCurlCommand(
    "-Ikv --resolve 'foo.com:{0}:127.0.0.1' https://foo.com:{0}/case3 --curves X25519MLKEM768".format(ts.Variables.ssl_port))
