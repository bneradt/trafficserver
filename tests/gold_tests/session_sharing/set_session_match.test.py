...
class SetSessionSharingTest:
    TestCounter = 0
    def __init__(self, TestSummary, sharingMatchValue):
        SetSessionSharingTest.TestCounter += 1
        self._MyTestCount = SetSessionSharingTest.TestCounter
        Test.Summary = TestSummary
        self._tr = Test.AddTestRun()
        self._sharingMatchValue = sharingMatchValue
        self._session_management_client = 'session_management_client.py'
        self.setupOriginServer()
        self.setupTS()
        self._ts.Setup.CopyAs(self._session_management_client, Test.RunDirectory)
    ...
    def runAndExpectSharing(self):
        self._tr.Processes.Default.Command = 'python3 {0} -p {1}'.format(
            self._session_management_client, self._ts.Variables.port)
...
sessionSharingTest.runAndExpectNoSharing()
