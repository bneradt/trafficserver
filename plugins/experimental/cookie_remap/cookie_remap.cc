    ...
    for (auto subop : subops) {
      ...
      if (subop_type == REGEXP) {
        ...
        if (ret >= 0) {
          ...
          // Store regex match strings (not views) so they persist after this function returns
          regex_match_strings.clear();
          regex_match_strings.reserve(regex_ccount + 1);
          for (int i = 0; i <= regex_ccount; i++) {
            regex_match_strings.emplace_back(string_to_match.substr(ovector[i * 2], ovector[i * 2 + 1] - ovector[i * 2]));
          }
