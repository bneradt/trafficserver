...
OperatorSetStatus::exec(const Resources &res) const
{
  ...
  case TS_HTTP_SEND_RESPONSE_HDR_HOOK:
    if (res.bufp && res.hdr_loc) {
      TSHttpHdrStatusSet(res.bufp, res.hdr_loc, static_cast<TSHttpStatus>(_status.get_int_value()), res.txnp, PLUGIN_NAME);
    ...
    break;
  default:
    TSHttpTxnStatusSet(res.txnp, static_cast<TSHttpStatus>(_status.get_int_value()), PLUGIN_NAME);
    break;
  }
...
OperatorSetRedirect::exec(const Resources &res) const
{
  if (res.bufp && res.hdr_loc && res.client_bufp && res.client_hdr_loc) {
    ...
    if (remap) {
      ...
      TSHttpHdrStatusSet(res.bufp, res.hdr_loc, status, res.txnp, PLUGIN_NAME);
...
OperatorSetBodyFrom::exec(const Resources &res) const
{
  ...
  if (createRequestString(_value.get_value(), req_buf, &req_buf_size) == TS_SUCCESS) {
    ...
    TSHttpTxnStatusSet(res.txnp, res.resp_status, PLUGIN_NAME);
  } else {
